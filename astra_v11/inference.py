"""
Inference module for protocol fingerprinting and intelligence gathering.
"""
...
def fingerprint_protocol(stats: StreamStats) -> InferenceResult:
    ...
    if stats.dominant_size == DOMINANT_PKT_SIZE:
        confidence += 0.4
        evidence.append(f"Dominant packet size ({DOMINANT_PKT_SIZE}) matches MAVLink heartbeat.")

    if stats.entropy_header < 4.0 and stats.entropy_payload > 7.0:
        confidence += 0.2
        evidence.append("Low header entropy / High payload entropy suggests partial encryption.")
