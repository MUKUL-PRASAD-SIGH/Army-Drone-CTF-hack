"""
Main entry point for the ASTRA-V11 SIGINT analysis pipeline.
...
def run_pipeline(args):
    ...
    logger.info("[bold blue]Starting ASTRA-V11 SIGINT Pipeline...[/]", extra={"markup": True})

    # 1. Ingestion
    logger.info("Phase 1: Ingesting raw stream...")
    if not args.input or not os.path.exists(args.input):
        logger.warning(f"Input file '{args.input}' not found. Using internal mock data.")
        import numpy as np
...
def print_summary_table(stats, result):
    ...
    table = Table(title="[bold cyan]ASTRA-V11 Pipeline Execution Summary[/]", title_justify="left")
...
def main():
    parser = argparse.ArgumentParser(description="ASTRA-V11 Drone-GCS UART SIGINT Pipeline")
