...
import pandas as pd
import pytest
from astra_v11 import protocol
...
def test_compute_sequence_counters():
    packets = [
        Packet(0, 0, b'', 0xAA, 0, 10, 0, b'', "", "0xAA"),
        Packet(1, 0, b'', 0xAA, 0, 11, 0, b'', "", "0xAA")
    ]
    df = protocol.compute_sequence_counters(packets)
    assert df.iloc[0]['sequence'] == 10
    assert df.iloc[1]['sequence'] == 11
